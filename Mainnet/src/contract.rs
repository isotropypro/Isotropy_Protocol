...
mod tests {
    ...
    fn emission_curve_matches_key_targets() {
        assert_eq!(emission_for_cycle(1).unwrap(), Uint128::new(10_000_000_000));
        assert_eq!(emission_for_cycle(365).unwrap(), Uint128::new(6_999_999_999));
        assert_eq!(emission_for_cycle(4002).unwrap(), Uint128::new(198_314_789));
        assert_eq!(emission_for_cycle(4003).unwrap(), Uint128::zero());
    }
    ...
    fn burn_and_cycle_advance_auto_stake_emission_into_real_cw20_supply() {
        ...
        assert_eq!(position.staked_balance, Uint128::new(10_000_000_000));
    ...
    fn claim_rewards_transfers_native_lunc_to_staker() {
        ...
        assert_eq!(balance.amount, quote.allocation_fee_amount);
    ...
    fn unstake_then_restake_supports_allowance_and_send_flows() {
        ...
        app.execute_contract(
            ...
            controller.clone(),
            &ExecuteMsg::Unstake {
                amount: Uint128::new(3_000_000_000),
            },
            &[],
        )
        .unwrap();
        ...
        let position = query_position_response(&app, &controller, "alice");
        assert_eq!(position.staked_balance, Uint128::new(8_500_000_000));
        assert_eq!(position.liquid_balance, Uint128::new(1_500_000_000));
        assert_eq!(
            query_token_balance(&app, &token, controller.as_str()),
            Uint128::new(8_500_000_000)
        );
    }
    ...
    fn rewards_split_across_existing_stakers_proportionally() {
        ...
        assert!(rounding_dust <= Uint128::new(10));
    ...
    fn token_supply_is_minted_only_as_cycles_advance() {
        ...
        let info_after: TokenInfoResponse = app
            ...
            .query_wasm_smart(&token, &Cw20QueryMsg::TokenInfo {})
            .unwrap();
        assert_eq!(info_after.total_supply, Uint128::new(10_000_000_000));
    }
}
